...
class SpriteSheetCreatorWindow(boring.dialog.DefaultDialog):
    def __init__(self, *args, **kws):
        self.__image = kws.pop('image')
        boring.dialog.DefaultDialog.__init__(self, *args, **kws)

    def body(self, master):
        self.canvas = boring.widgets.ExtendedCanvas(master, bd=0, highlightthickness=0, bg='black')
        self.canvas.pack(expand='yes', fill='both')
        self.canvas.bind('<1>', self.__switch_color, '+')
        self.__photo = ImageTk.PhotoImage(self.__image)
        self.__imagedraw = boring.draw.ImageDraw(self.canvas, 0, 0, self.__photo, anchor='nw')
...
def execute(ide):
    ...
    if images:
        ...
        SpriteSheetCreatorWindow(
            ide, image=final_image
        )